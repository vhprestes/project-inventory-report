...
def test_decorar_relatorio():
    simple = ColoredReport(SimpleReport).generate(mock)
    complete = ColoredReport(CompleteReport).generate(mock)
    assert ("\033[31m" in simple) is True
    assert ("\033[31m" in complete) is True
    assert ("\033[32m" in complete) is True
    assert ("\033[32m" in simple) is True
    assert ("\033[36m" in simple) is True
    assert ("\033[36m" in complete) is True
